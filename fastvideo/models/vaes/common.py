...
class ParallelTiledVAE(ABC):
    ...
    def spatial_tiled_encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self._merge_spatial_tiles(rows, blend_height, blend_width, tile_latent_stride_height,
    ...
    def parallel_tiled_decode(self, z: torch.FloatTensor) -> torch.FloatTensor:
        ...
        for local_idx, global_idx in enumerate(range(start_tile_idx, end_tile_idx)):
            ...
            tile = z[:, :, t_start:t_start + tile_latent_min_num_frames + 1, h_start:h_start + tile_latent_min_height,
        ...
        for i, tem_data in enumerate(data):
            slice_data = self._merge_spatial_tiles(tem_data, blend_height, blend_width, self.tile_sample_stride_height,
    ...
    def spatial_tiled_decode(self, z: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(0, height, tile_latent_stride_height):
            ...
            for j in range(0, width, tile_latent_stride_width):
                ...
                row.append(decoded)
            rows.append(row)
        return self._merge_spatial_tiles(rows, blend_height, blend_width, self.tile_sample_stride_height,
                                         self.tile_sample_stride_width)
    ...
    def tiled_encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, tile in enumerate(row):
            if i > 0:
                tile = self.blend_t(row[i - 1], tile, blend_latent_num_frames)
