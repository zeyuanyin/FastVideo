...
from .flash_attn_bsa_varlen_mask import (_attn_fwd_bsa_varlen, _attn_fwd_bsa_varlen_align,
                                         _attn_bwd_dkdv_bsa_varlen_wrapper, _attn_bwd_dq_bsa_varlen_wrapper,
                                         _attn_bwd_dq_bsa_varlen_align_wrapper, configs_fwd_bsa_varlen_preset,
                                         configs_fwd_bsa_varlen_align_preset, configs_bwd_dkdv_bsa_varlen_preset,
...
def get_select_indices_topk_from_score(score, sparsity):
    ...
    block_indices_lens = torch.full((block_indices.shape[0], block_indices.shape[1], block_indices.shape[2]),
                                    num_selected,
                                    dtype=torch.int32,
                                    device=block_indices.device)
...
def attn_fwd_bsa_varlen_triton(q, k, v, sm_scale, block_indices, block_indices_lens, chunk_size_q, chunk_size_k,
                               sparsity):
    ...
    if chunk_size_k > 128:
        ...
        kernel_config = {} if os.environ.get('TRITON_AUTOTUNE_ENBALE',
                                             '0') == '1' else configs_fwd_bsa_varlen_preset[config_key]
    else:
        fwd_func = _attn_fwd_bsa_varlen_align
        kernel_config = {} if os.environ.get('TRITON_AUTOTUNE_ENBALE',
                                             '0') == '1' else configs_fwd_bsa_varlen_align_preset[config_key]
...
def attn_bwd_bsa_varlen_triton(do, q, k, v, o, dq, dk, dv, sm_scale, M, block_indices, block_indices_lens, chunk_size_q,
                               chunk_size_k, sparsity):
    ...
    block_indices_k, block_indices_k_lens = create_indices_k_from_indices_q_varlen(block_indices=block_indices,
    ...
    kernel_config = {} if os.environ.get('TRITON_AUTOTUNE_ENBALE',
                                         '0') == '1' else configs_bwd_dkdv_bsa_varlen_preset[config_key]
    ...
    if chunk_size_k > 128:
        ...
        kernel_config = {} if os.environ.get('TRITON_AUTOTUNE_ENBALE',
                                             '0') == '1' else configs_bwd_dq_bsa_varlen_preset[config_key]
    else:
        bwd_dq_func = _attn_bwd_dq_bsa_varlen_align_wrapper
        kernel_config = {} if os.environ.get('TRITON_AUTOTUNE_ENBALE',
                                             '0') == '1' else configs_bwd_dq_bsa_varlen_align_preset[config_key]
...
def make_block_indices_varlen_cp_list(block_indices, cp_size, num_blocks_k_full):
    ...
    for i in range(cp_size):
        ...
        block_indices_tmp[
            block_indices_tmp <
            0] = num_blocks_per_rank  # block_indices_tmp < 0 indicate invalid indices, set them to num_blocks_per_rank in order to sort them to the tail, so that the first N elements of the block_indices indicated by block_indices_lens are valid
...
class _attention_bsa(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, chunk_size_q, chunk_size_k, sparsity, cdf_threshold, sm_scale, use_tma=False):
        ...
        o, lse = attn_fwd_bsa_varlen_triton(q, k, v, sm_scale, block_indices, block_indices_lens, chunk_size_q,
                                            chunk_size_k, sparsity)
    ...
    def backward(ctx, do):
        ...
        attn_bwd_bsa_varlen_triton(do, q, k, v, o, dq, dk, dv, ctx.sm_scale, lse, block_indices, block_indices_lens,
