...
def map_lora_module(module_path: str) -> list[tuple[str, str]]:
    ...
    if match:
        b = match.group(1)
        return [(f"blocks.{b}.self_attn.to_q", "q"), (f"blocks.{b}.self_attn.to_k", "k"),
    ...
    if match:
        b = match.group(1)
        return [(f"blocks.{b}.cross_attn.to_k", "k"), (f"blocks.{b}.cross_attn.to_v", "v")]
...
def convert_lora_weights(source_weights: dict[str, torch.Tensor], lora_name: str) -> dict[str, torch.Tensor]:
    ...
    for module_path, weight_keys in modules.items():
        ...
        if "alpha_scale" in weight_keys:
            ...
            alpha_scale = alpha_scale_tensor.item() if alpha_scale_tensor.numel() == 1 else float(
                alpha_scale_tensor.mean())
...
def main():
    parser = argparse.ArgumentParser(description="Convert LongCat weights to FastVideo native format")
    parser.add_argument(
        "--source",
        type=str,
        required=True,
