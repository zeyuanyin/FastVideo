...
try:
    ...
    def _unsupported(*args, **kwargs):
        raise ImportError("flash-attn is not installed. Please install it, e.g., `uv pip install flash-attn`.")
...
def _select_threshold_query_head(gate: torch.Tensor, valid_gate_mask: torch.Tensor, gate_self_chunk_mask: torch.Tensor,
...
def _select_threshold_block(gate: torch.Tensor, valid_gate_mask: torch.Tensor, gate_self_chunk_mask: torch.Tensor,
                            simsum_threshold: float) -> torch.Tensor:
    """
    ...
    cutoff_flat = torch.where(any_cond,
                              cond_flat.float().argmax(dim=1), torch.full_like(any_cond, fill_value=HS - 1))  # (C,)
...
def _select_threshold_overall(gate: torch.Tensor, valid_gate_mask: torch.Tensor, gate_self_chunk_mask: torch.Tensor,
                              simsum_threshold: float) -> torch.Tensor:
    """
    ...
    overall_denom = torch.where(overall_denom <= eps, torch.tensor(1.0, device=gate.device, dtype=gate.dtype),
                                overall_denom)
...
def _select_threshold_head_global(gate: torch.Tensor, valid_gate_mask: torch.Tensor, gate_self_chunk_mask: torch.Tensor,
                                  simsum_threshold: float) -> torch.Tensor:
    """
...
class MixedAttention(torch.autograd.Function):
    ...
    def forward(
        ...
        moba_attn_lse = (moba_attn_lse.view(-1).sub(max_lse_1d.index_select(
    ...
    def backward(ctx, d_output):
        ...
        _ = _flash_attn_varlen_backward(dout=d_output,
                                        q=q,
                                        k=k,
                                        v=v,
                                        out=output,
...
def moba_attn_varlen(
    ...
    gate_seq_idx = torch.arange(seqlen, device=q.device,
                                dtype=torch.int32).unsqueeze(0).expand(num_filtered_chunk, seqlen)
    ...
    if select_mode == 'topk':
        ...
        if threshold_type == 'query_head':
            ...
            raise ValueError(f"Invalid threshold_type for topk: {threshold_type}. "
...
def process_moba_input(
    ...
    if isinstance(chunk_size, float) or isinstance(chunk_size, int):
        ...
        assert isinstance(chunk_size,
                          (tuple, list)), f"chunk_size should be a tuple, list, or int, now it is: {type(chunk_size)}"
        if len(chunk_size) == 2:
            assert patch_resolution[1] % chunk_size[0] == 0 and patch_resolution[2] % chunk_size[
            ...
            assert patch_resolution[0] % chunk_size[0] == 0 and patch_resolution[1] % chunk_size[
                1] == 0 and patch_resolution[2] % chunk_size[
                    2] == 0, f"patch_resolution {patch_resolution} should be divisible by 3d chunk_size {chunk_size}"
            nct, nch, ncw = patch_resolution[0] // chunk_size[0], patch_resolution[1] // chunk_size[
                1], patch_resolution[2] // chunk_size[2]
            x = rearrange(x,
